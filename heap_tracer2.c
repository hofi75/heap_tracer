...
static void prepare(void)
{
	...
	printf( "heap tracer envitonment settings '%s'\n", htenv);
	if ( ( p = strstr( htenv, "progname=")) != NULL)
	{
	    sprintf( temp, "progname=%s", __progname);
	    if ( ( p = strstr( htenv, temp)) == NULL) return;
	}

	memset( report_path, 0, sizeof( report_path));
	if ( ( p = strstr( htenv, "report_path=")) != NULL)
	    for( d = report_path, p += 12; *p != '\0' && *p != ' '; p++, d++) *d = *p;
    ...
    if ( is_locking) pthread_mutex_init( &htc->lock, NULL);
...
static int compare_ae( const void *p1, const void *p2)
{
    allocate_entry_t *ae1 = ( allocate_entry_t *) p1;
    allocate_entry_t *ae2 = ( allocate_entry_t *) p2;
...
static void unload(void)
{
    ...
    qsort( ( void *) htc->table, htc->unused, sizeof( allocate_entry_t), compare_ae);

    for ( sum = i = 0, ae = htc->table; i < htc->unused; i++, ae += 1)
    {
	if ( ae->addr == NULL) continue;

	trc( "[%d][ID=%lld] area at %p with size %ld not freed\n", i, ae->id, ae->addr, ae->size);
	trc( "     backtrace = %s\n", ae->backtrace);
...
void generate_backtrace_string( char *result, const void *caller)
{
    ...
    *result = '\0';
    ...
    for ( l = SIZE_BACKTRACE, p = result, i = 2; i < size && messages[i] != NULL; ++i)
    {
...
allocate_entry_t * table_add_entry( char *addr, size_t size)
{
    ...
    // save in table
    ae = htc->table + htc->unused;
    memcpy( ae->eyec, "%%AE", 4);
    ae->real_addr	= addr;
    if ( is_check_boundaries)
    {
	ae->addr	= addr + SIZE_CHECK_AREA;
	ae->trailer_cs	= addr + size + SIZE_CHECK_AREA;
    } else
	ae->addr	= addr;
    ae->size		= size;
    ae->id		= htc->id ++;
...
void * table_remove_entry( void *addr, const char *backtrace)
{
    ...
    if ( is_trace_module) trc( "%s <-- (rslt=%p)\n", __FUNCTION__, rslt);
...
static void *my_malloc_hook( size_t size, const void *caller)
{
    ...
    if ( is_check_boundaries) alloc_size = size + SIZE_CHECK_AREA * 2;
    ...
    // return address
    return ( is_check_boundaries) ? ae->addr : addr;
}
...
static void *my_realloc_hook ( void *ptr, size_t size, const void *caller)
{
    ...
    ptr = table_remove_entry( ptr, bts);

    if ( is_check_boundaries) alloc_size = size + SIZE_CHECK_AREA * 2;
			 else alloc_size = size;

    if ( old_realloc_hook != NULL)
	addr = (__ptr_t) (*old_realloc_hook)( ptr, alloc_size, caller);
    else
	addr = (__ptr_t) realloc( ptr, alloc_size);
...
void __dump( const char *title, char *addr, size_t len)
{
	...
	case 19:
	case 23:
	case 27: strcat( s1, " "); break;
	case 15: strcat( s1, " -  "); break;
	}
      s4[0] = ( isprint( *p)) ? *p : '.';
      strcat( s2, s4);
      p ++;
...
int set_checkstring( allocate_entry_t *ae)
{
    ...
    sprintf( temp, "#*#*%p<***>%.10ld#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*", ae->addr, ae->size);
